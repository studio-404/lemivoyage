<?php 
session_start();
if(@$_SESSION["token"]!=@$_GET["token"] && !isset($_GET["l"])){
	exit();
}
require_once("../../app/functions/map.php"); 
$map = new functions\map();
$markerData = $map->generateMarkers($_GET["l"]);
?>
<!DOCTYPE html>
<html>
  <head>
  ...
  <body>
    ...
    <script>
		...
		var marker2;
		var map;
		var counts = <?=$markerData["counts"]+1?>;
